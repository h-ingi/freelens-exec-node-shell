...
`1.10.3-14`는 설정 IPC 요청·응답을 검증한 JSON 문자열로 전달합니다.
MobX Proxy 등 복제 불가능한 객체를 직접 Electron IPC에 넘기지 않도록,
지원하는 설정 필드만 명시적으로 복사합니다. main에서도 요청을 다시 검증합니다.
테스트는 V8 직렬화·역직렬화 경계를 적용하여 반응형 설정 객체의 복제 실패와 수정 경로를 확인합니다.
이는 Windows FreeLens에서의 실제 저장·제한시간 검증을 대신하지 않습니다.

사용자 환경에서는 노드 셸 접속, exit 정리, 탭 X 정리, 세션 화면, Stop 기능(1~5번)이 확인됐습니다.
설정 저장 수정 후 제한시간(6번), 재시작 후 설정 유지(7번), 권한 진단(8번)을 이어서 검증합니다.

다음 항목은 사용자의 요청에 따라 기능 검증 완료 후 진행하며, 이번 저장 오류 수정에는 포함하지 않습니다.

- 설정 UI를 FreeLens 기본 설정과 유사한 입력란·간격·레이블 형태로 개선
- 확장 및 설정 화면 이름을 `freelens-exec-node-shell`로 표시 (`@h-ingi/` 제거)
- 노드 셸 프롬프트에 노드명 또는 hostname과 현재 경로 표시

이름 변경 시 기존 설치와 저장 설정의 이전 방법도 함께 검토합니다.
