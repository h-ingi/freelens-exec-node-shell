...
describe("extensions page tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!process.env.EXTENSION_PATH) throw new Error("EXTENSION_PATH must point to the package built by CI");
    ...
    expect(installedExtensionName).toBe("freelens-exec-node-shell");
